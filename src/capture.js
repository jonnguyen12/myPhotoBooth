...
window.addEventListener('DOMContentLoaded', _ => {
  ...
  recordEl.addEventListener('click', _ => {
    recordEl.setAtttribute('disabled', 'disabled')
  ...
  photosEl.addEventListener('click', evt => {
    ...
    if (index > -1) {
      if (isRm) ipc.send('image-remove', index)
    }
    else {
...
ipc.on('image-removed', (evt, index) => {
  document.getElementById('photos').removeChild(Array.from(document.querySelectorAll('.photos'))[index])
})
